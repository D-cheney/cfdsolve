...
slug: cfd-turbulence-k-omega-sst-diagnosis-validation
title: "k–omega SST 模型：结果诊断与可信度验证"
summary: "把 SST 结果的可信度拆成可算的量：由 k 与 omega 算出 F2 过渡距离并判断剪切应力限制是否生效、用对数律一致性检验限制器的实现、扫自由来流 omega 的敏感度、对升力与阻力分别做网格收敛指数。"
category:
...
seo:
  title: "k–omega SST 模型：结果诊断与可信度验证"
  description: "把 SST 结果的可信度拆成可算的量：由 k 与 omega 算出 F2 过渡距离并判断剪切应力限制是否生效、用对数律一致性检验限制器的实现、扫自由来流 omega 的敏感度、对升力与阻力分别做网格收敛指数。"
  keywords:
    - "k–omega SST 模型"
    - "结果诊断与可信度验证"
    - "剪切应力限制"
...
# k–omega SST 模型：结果诊断与可信度验证

SST 出问题时多数人先怀疑网格，更常见的失效却是剪切应力限制根本没被触发，而这一点完全可以用手算判断。本文给出四个可量化的诊断：由 $k$ 与 $\omega$ 算出 $F_2$ 过渡距离、用对数律一致性检验限制器、扫自由来流 $\omega$ 看结果漂移、对升力与阻力分别做网格收敛指数。算例为 NACA 4412，弦长 $c = 1\,\mathrm{m}$、来流 $U_\infty = 23\,\mathrm{m/s}$、$\nu = 1.5\times10^{-5}\,\mathrm{m^2/s}$、攻角 $13.87^\circ$，即 $Re = 1.53\times10^{6}$。
...
2. Coles D., Wadcock A. J., "Flying-hot-wire study of flow past an NACA 4412 airfoil at maximum lift," *AIAA Journal*, 1979.
3. Spalart P. R., Rumsey C. L., "Effective inflow conditions for turbulence models in aerodynamic calculations," *AIAA Journal*, 2007.
4. Celik I. B., Ghia U., Roache P. J., Freitas C. J., Coleman H., Raad P. E., "Procedure for estimation and reporting of uncertainty due to discretization in CFD applications," *ASME Journal of Fluids Engineering*, 2008.
