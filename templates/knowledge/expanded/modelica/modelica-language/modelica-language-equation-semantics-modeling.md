...
Modelica 的 `equation` 段不指定计算顺序，它只声明变量之间必须同时成立的关系；工具负责展开层次、生成连接方程、做符号排序并选择状态量。判断一个模型是否正确，靠的是展开后的方程—变量配平与结构非奇异性，而不是代码能否通过语法检查。本文只讨论 equation 段的语义、平衡与 `connect` 生成规则，不涉及 Media/Fluid 的物性闭合与求解器设置。方程类报错常被当成"编译器不听话"，实际上每一条消息都对应一个可判定的结构或数值事实。诊断的关键不是改参数，而是先把报错归到结构、数值、指标三类之一，再用最小模型复现。本文以翻译期与初始化期的真实消息为线索，给出一个指标为 3 的杆摆算例作为可复算对照。
...
7. Modelica Association. *Modelica Language Specification, Version 3.6*, 2023 — 第 4 章 Balanced Models 与第 8 章 Equation Operators 定义匹配与 connect 语义。
8. Pantelides, C. C. "The Consistent Initialization of Differential-Algebraic Systems." *SIAM J. Sci. Stat. Comput.*, 9(2), 1988.
9. Cellier, F. E., Kofman, E. *Continuous System Simulation*. Springer, 2006 — 第 7 章给出指标定义与约减算法。
10. Mattsson, S. E., Söderlind, G. "Index Reduction in Differential-Algebraic Equations Using Dummy Derivatives." *SIAM J. Sci. Comput.*, 14(3), 1993.
11. Fritzson, P. *Principles of Object-Oriented Modeling and Simulation with Modelica 3.3*. Wiley-IEEE Press, 2015 — 第 8 章讨论翻译期结构检查。
12. OpenModelica User's Guide, v1.22, 2023 — 附录列出 `Translation Error` 与 `singular Jacobian` 消息的含义。
