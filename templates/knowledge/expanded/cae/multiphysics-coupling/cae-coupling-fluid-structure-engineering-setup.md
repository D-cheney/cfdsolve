...
水中的薄钢板与空气中的桥梁翼段，几何与流速可以完全相同，耦合行为却截然不同：前者几乎必然需要隐式强耦合，后者用每步交换一次数据的显式格式即可稳定。差别来自附加质量比，而不是软件默认值。结构振幅、界面残差与守恒量经常给出互相矛盾的信号：残差收敛了但振幅还在漂移，或者振幅稳定但界面合力不为零。诊断的任务是判断这些信号里哪一个是物理、哪一个是数值伪影。
...
7. Turek S., Hron J., "Proposal for numerical benchmarking of fluid–structure interaction between an elastic object and laminar incompressible flow," in *Fluid–Structure Interaction*, Springer LNCSE 53, 2006.
8. Förster C., Wall W.A., Ramm E., "Artificial added mass instabilities in sequential staggered coupling of nonlinear structures and incompressible viscous flows," *Computer Methods in Applied Mechanics and Engineering*, 196, 2007.
9. Roache P.J., "Perspective: a method for uniform reporting of grid refinement studies," *ASME Journal of Fluids Engineering*, 116, 1994.
