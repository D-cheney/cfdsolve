...
SPH 里存在两类完全不同的黏性：一类是代表真实流体动量输运的层流黏性项，另一类是为抑制冲击振荡与粒子飞散而人为加入的数值黏性。把二者混为一谈会让雷诺数失去物理意义，也会让"调黏性"变成掩盖其他误差的手段。黏性设置之所以容易出错，是因为 $\alpha_\Pi$ 这类系数看上去只是"稳定性参数"，实际却等价于一个可换算的运动黏度。落地时只有四步：先由流体物性填层流黏度，再由目标等效黏度反算 $\alpha_\Pi$，按马赫数决定 $\beta_\Pi$，最后用 Balsara 开关把数值耗散限制在激波处。"结果太黏"至少有三个来源：真实物性、Monaghan 型人工黏性、分辨率不足带来的隐式耗散。三者都让动能单调下降，单看能量曲线无法区分。
...
8. Taylor G.I., Green A.E., *Mechanism of the production of small eddies from large ones*, Proceedings of the Royal Society A, Vol. 158, 1937.
