...
三次样条核的工程配置只需要锁定三个输入：粒子间距 $\Delta p$、平滑长度比 $\eta$、支持半径倍数 $\kappa$。本文用 $\Delta p=5$ mm、$\eta=1.1$、$\kappa=2$ 的三维算例，把这三个数推成 $h=5.5$ mm、$r_c=11$ mm、内部邻居数 44.6，并给出可直接抄用的核函数与导数实现、链表格元边长和配套时间步。三次样条核的多数"数值噪声"抱怨其实可以归到三个可测量上：核的二阶矩给出的有效平滑宽度、离散归一化量 $S_i$ 的分布、以及拉伸区里 $r<0.5\Delta p$ 的粒子对数量。
...
8. Price D.J., *Smoothed particle hydrodynamics and magnetohydrodynamics*, Journal of Computational Physics, 231, 759–794, 2012.
