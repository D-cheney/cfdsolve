...
在线统计的价值在于不必保存全部时间步的场：`fieldAverage` 边算边累加，跑完就得到平均值与脉动均方值，磁盘占用只有若干标量文件。代价是统计窗口一旦设错，重算代价与整个算例相当。选型、时长、归一化这三件事必须在启动前定下来。一个不带误差棒的时间平均值没有意义。`fieldAverage` 和 `forceCoeffs` 给出的是数字，判断这些数字能不能用，需要回答三个问题：样本之间是否独立、均值的不确定度是多少、以及统计对象本身有没有改变解。前两个是统计问题，第三个是数值问题，验证方法完全不同。
...
7. Pope S. B. Turbulent Flows. CUP, 2000, ch. 12 (statistical description of turbulence).
8. Sreenivasan K. R., Antonia R. A., Danh H. Q. Temperature dissipation fluctuations in a turbulent boundary layer. Physics of Fluids, 20(8):1238–1249, 1977.
9. Oliver T. A., Malaya N., Ulerich R., Moser R. D. Estimating uncertainties in statistics computed from direct numerical simulation. Physics of Fluids, 26(3):035101, 2014.
10. Norberg C. Fluctuating lift on a circular cylinder. Journal of Fluids and Structures 17(1), 2003.
11. OpenCFD Ltd. OpenFOAM User Guide v11. 2024. Chapter: Function objects.
