...
slug: "meshfree-jilong-overview"
title: "吉隆无网格模型：版本与文件导航"
summary: "梳理吉隆模型的一维基线、2.5D 展示和三维 SPH 版本，连接输入文件、配置与复现步骤。"
...
> 本文由项目原始资料转为站内阅读版；原文中的本机路径和历史运行记录仅供追溯。[下载原始文档](/meshfree/documents/jilong-overview.md) · [查看专题与模型文件](/meshfree)。

# 吉隆口岸冰川融水—泥石流无网格模型

## 最新入口：48 万粒子云图版

真实动力粒子增加为 480000（24000 的 20 倍），源区体积和外形保持不变；仅生成四个角度的连续速度云图视频，其余为静态图片。配置、后台运行及展示页说明见 [48 万粒子云图模板](/knowledge/meshfree-cloud-480k)。完整时长以运行目录的完成状态为准，短时预览与完整结果分开保存。

## 当前入口：三维 SPH

整体升级的三维求解器为 `src/simulate_sph3d.py`，配置为 `config/sph3d.json`。采用独立 xyz 运动、三维压力及正则化宾汉黏性，默认 24000 个体积粒子，目标时长 90 分钟。运行方法、边界假设和验证要求见 [三维可复用模板](/knowledge/meshfree-sph3d-model)。实际完成情况读取 D 盘对应运行目录的 `summary.json`。
