...
slug: "meshfree-reusable-workflow"
title: "无网格泥石流模型：场地替换与复现流程"
summary: "从场地数据替换、情景配置到历史模型运行与成果交付的完整操作参考，复用前需核对本机路径和环境。"
...
> 本文由项目原始资料转为站内阅读版；原文中的本机路径和历史运行记录仅供追溯。[下载原始文档](/meshfree/documents/reusable-workflow.md) · [查看专题与模型文件](/meshfree)。

# 冰川融水—泥石流无网格仿真可重复利用模板

> 三维升级入口：请使用 [THREE_DIMENSIONAL_TEMPLATE.md](/knowledge/meshfree-sph3d-model)。本文后续保留的是旧一维动力与 2.5D 显示模板，不适用于解释新三维结果。
