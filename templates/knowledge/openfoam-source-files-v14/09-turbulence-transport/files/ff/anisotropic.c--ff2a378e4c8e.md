...
slug: "openfoam-v14-file-ff2a378e4c8e"
title: "OpenFOAM 14 源码解析：anisotropic.C"
summary: "该文件实现 `anisotropic` 的运行时工厂选择，根据类型名创建具体实现。"
...
source_path: "src/ThermophysicalTransportModels/solid/anisotropic/anisotropic.C"
tags: [OpenFOAM14, 源码解析, 湍流与输运]
---

# OpenFOAM 14 源码解析：anisotropic.C

> 本页由静态分析生成，再按可识别的 OpenFOAM 模式补充中文算法说明。它用于源码导航，不替代编译、调试和算例验证。

## 1. 文件定位

- 源码路径：`src/ThermophysicalTransportModels/solid/anisotropic/anisotropic.C`
- 功能分类：湍流与输运
- 文件类型：C/C++ 或词法/语法源文件
- 规模：527 行
- 文件标识：`ff2a378e4c8e`
...
该文件实现 `anisotropic` 的运行时工厂选择，根据类型名创建具体实现。
...
7. **并行归约/通信**：在处理器间交换或归约局部量，形成全局一致结果。
8. **分布式映射**：依据全局到局部寻址重排和交换数据。
9. **网格变化响应**：在拓扑或点位置变化后重建寻址、缓存和依赖场。
