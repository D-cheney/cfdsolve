...
slug: "openfoam-v14-file-b879f8e6e3fb"
title: "OpenFOAM 14 源码解析：correctPressure.C"
summary: "该文件实现 `correctPressure` 的运行时工厂选择，根据类型名创建具体实现。"
...
source_path: "applications/modules/isothermalFluid/correctPressure.C"
...
- 源码路径：`applications/modules/isothermalFluid/correctPressure.C`
- 功能分类：模块化求解器
- 文件类型：C/C++ 或词法/语法源文件
- 规模：274 行
- 文件标识：`b879f8e6e3fb`
...
该文件实现 `correctPressure` 的运行时工厂选择，根据类型名创建具体实现。
...
| 名称 | 源码行 |
|---|---:|
| `Foam::solvers::isothermalFluid::correctPressure` | 47 |
...
11. **压力速度耦合**：在动量预测、压力校正和外/内迭代之间协调场更新。
12. **边界回写**：内部场更新后重新执行各 patch 的边界条件计算。
