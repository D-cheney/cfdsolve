...
function runMigrations(db: DatabaseSync) {
  ...
  // Acquire the write lock before reading the migration ledger. Otherwise two
  // processes starting against the same new database can both decide to replay
  ...
  try {
    ...
    const latestSupported = databaseMigrations.at(-1)?.version ?? 0
    const latestApplied = applied.size ? Math.max(...applied) : 0
    if (latestApplied > latestSupported) {
      throw new Error(`数据库 Schema ${latestApplied} 高于当前程序支持的版本 ${latestSupported}`)
    }
...
function seedDatabase(db: DatabaseSync) {
  ...
  try {
    ...
    db.prepare(`INSERT OR IGNORE INTO users (id, email, username, display_name, status)
      VALUES (?, ?, ?, ?, 'ACTIVE')`).run('user-codex-generated', 'knowledge-bot@localhost.invalid', 'codex-generated', '源码知识库生成器')
