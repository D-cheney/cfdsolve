  ...
  await db.transaction(async () => {
    if (body.user?.name) {
      await db.run(`UPDATE users SET display_name = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
        String(body.user.name).slice(0, 80), DEMO_USER_ID)
    }

    if (Array.isArray(body.bookmarks)) {
      await db.run('DELETE FROM bookmarks WHERE user_id = ?', DEMO_USER_ID)
      for (const key of body.bookmarks.slice(0, 500)) {
        const value = String(key).slice(0, 300)
        await db.run(`INSERT INTO bookmarks
          (user_id, resource_type, resource_key) VALUES (?, ?, ?)`, DEMO_USER_ID, value.startsWith('formula-') ? 'formula' : 'content', value)
      }
    }
    ...
    if (Array.isArray(body.tasks)) {
      for (const item of body.tasks.slice(0, 500)) {
        const tool = await db.get(`SELECT id FROM simulation_tools WHERE slug = ?`, String(item.tool || '')) as { id: string } | undefined
        ...
        await db.run(`INSERT INTO simulation_tasks
    ...
    if (Array.isArray(body.projects)) {
      for (const item of body.projects.slice(0, 100)) {
        ...
        await db.run(`INSERT INTO modelica_projects
          ...
          id, DEMO_USER_ID, String(item.name).slice(0, 200), String(item.slug).slice(0, 200), String(item.template).slice(0, 200), status, lastCompile, updatedAt)
        await db.run(`INSERT INTO modelica_files
          ...
          `${id}-main`, id, `${String(item.template || 'Model')}.mo`, String(item.code || '').slice(0, 2_000_000), updatedAt)
      }
    }
